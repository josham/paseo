...
Appearance settings (theme, UI/mono font family, font sizes, content measure, syntax-highlight theme) are owned by `packages/app/src/appearance`. Its provider subscribes once and synchronizes Unistyles when settings or plugin contributions change. `applyAppearance` patches every key in `REGISTERED_THEMES`, returning `{ ...theme, fontFamily, fontSize, lineHeight, layout, colors.syntax }`.
...
- **Non-React consumers need an explicit mirror.** `theme.layout.maxContentWidth` also drives virtualized height estimation, which runs in plain modules that cannot read a theme. `applyAppearance` pushes the committed value to `utils/content-estimate-width.ts` in the same pass. Reading a layout token at module scope pins the estimator to the authored default; read it per call.
