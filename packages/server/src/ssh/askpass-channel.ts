...
import { shellQuote } from "@getpaseo/protocol/ssh-lifecycle";
import { sshAskpassEnv, type SshProcessEnv } from "@getpaseo/protocol/ssh-transport";

/**
 * What SSH is asking for. The raw prompt is the only signal it gives, and the
 * three need visibly different treatment: one unlocks a local key file, one is
 * the remote account's password, and one is not a secret at all — it is the
 * host-key fingerprint, which must be shown and confirmed rather than typed
 * into a masked field. Classifying here means the UI does not have to parse
 * English prompt text.
 */
export type AskpassKind = "passphrase" | "password" | "confirm";

export interface AskpassRequest {
  /** SSH's own prompt, verbatim — carries the key path, `user@host`, or the fingerprint. */
  prompt: string;
  kind: AskpassKind;
}
...
export interface AskpassChannelOptions {
   ...
   * aborts {@link AskpassChannel.signal}, which callers use to stop
...
export interface AskpassChannel {
  ...
  /**
   * Environment for one `ssh` child. Each call tags the child with a fresh
   * session id, which is what lets an answer be reused across processes but
   * never within one — see {@link createAskpassChannel}.
   */
  askpassEnv(baseEnv: SshProcessEnv): SshProcessEnv;
...
/** Names the `ssh` process a prompt came from. See {@link createAskpassChannel}. */
const SESSION_ENV_VAR = "PASEO_ASKPASS_SESSION";

/**
 * Host-key confirmation reaches askpass too once `BatchMode` is off (OpenSSH
 * 8.4+ with `SSH_ASKPASS_REQUIRE=force`). Its answer is the literal word
 * "yes", so mistaking it for a password would mean asking the user to type a
 * fingerprint into a masked field.
 */
export function classifyAskpassPrompt(prompt: string): AskpassKind {
  if (/\(yes\/no/i.test(prompt) || /authenticity of host/i.test(prompt)) return "confirm";
...
const session = process.env[${JSON.stringify(SESSION_ENV_VAR)}] ?? "";

const socket = net.connect(socketPath);
let buffered = "";

socket.on("connect", () => {
  socket.write(JSON.stringify({ prompt, session }) + "\\n");
});
...
interface AskpassMessage {
  prompt: string;
  session: string;
}

function parseAskpassMessage(line: string): AskpassMessage | null {
  try {
    const parsed: unknown = JSON.parse(line);
    if (!parsed || typeof parsed !== "object") return null;
    const record = parsed as { prompt?: unknown; session?: unknown };
    return {
 ...
 * an account password.
 *
 * Setting a host up and then tunnelling to it are two `ssh` processes, so the
 * channel replays an answer it already has rather than asking twice for the
 * same password. It replays only to a *different* process: within one `ssh`,
 * a repeated prompt means the last answer was wrong, and replaying it would
 * spend the remaining attempts on a password already known to be bad. The
 * session id in {@link AskpassChannel.askpassEnv} is what tells the two apart.
 *
...
export function createAskpassChannel(options: AskpassChannelOptions): Promise<AskpassChannel> {
  ...
  /** The last answer the user gave, and the ssh process that asked for it. */
  let answered: { prompt: string; session: string; secret: string } | null = null;

  const resolveSecret = async (message: AskpassMessage): Promise<string | null> => {
    if (
      answered &&
      answered.prompt === message.prompt &&
      answered.session !== message.session &&
      message.session !== ""
    ) {
      return answered.secret;
    }
    const secret = await options.onPrompt({
      prompt: message.prompt,
      kind: classifyAskpassPrompt(message.prompt),
    });
    answered = secret === null ? null : { ...message, secret };
    return secret;
  };
  ...
  return new Promise<AskpassChannel>((resolve, reject) => {
    ...
    server.listen(socketPath, () => {
      ...
      server.on("error", () => {
        // A dead channel just means prompts go unanswered; the caller's own
        // failure path reports that far more usefully than a crash here.
      });
      resolve({
        askpassPath,
        askpassEnv: (baseEnv) => ({
          ...sshAskpassEnv(baseEnv, askpassPath),
          [SESSION_ENV_VAR]: randomUUID(),
        }),
