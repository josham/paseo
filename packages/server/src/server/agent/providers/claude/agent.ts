...
export class ClaudeAgentClient implements AgentClient {
  ...
  async listImportableSessions(
    ...
    const candidates = await collectRecentClaudeSessions(files, sessionsRoot, scanLimit, {
...
async function readClaudeSidechainHistory(
  ...
  for (const entry of await files.listFiles(workflowDirectory, {
    suffix: ".json",
    maxDepth: 1,
  })) {
    // A partial or unreadable run summary must not fail the rest of history ingestion.
    const contents = await files.readFile(entry.path);
    if (contents !== null) history.workflowContents.push(contents);
  }
  if (!(await files.exists(sidechainDirectory))) return history;

  for (const entry of await files.listFiles(sidechainDirectory, {
    suffix: ".jsonl",
    maxDepth: SIDECHAIN_SEARCH_DEPTH,
  })) {
    const contents = await files.readFile(entry.path);
    if (contents !== null)
      recordClaudeSidechainContents(history, sidechainDirectory, entry.path, contents);
  }
  ...
  for (const entry of await files.listFiles(sidechainDirectory, {
    ...
    const metaMatch = CLAUDE_SUBAGENT_META_FILE.exec(path.basename(entry.path));
    if (!metaMatch?.[1]) continue;
    // Undocumented internals: a missing or unreadable sidecar must never fail ingestion.
    const raw = await files.readFile(entry.path);
    if (raw === null) continue;
    const meta = parseClaudeSubagentMeta(raw);
