...
const exceptions = {
  ...
  // Added by pr/2453-devcontainer, which predates this map: build output for a
  // throwaway probe container, correlated to the probe request that asked for it.
  // The workspace's own container streams `container.lifecycle.progress`, which
  // is keyed by workspace and carries no requestId, so it is not a reply at all
