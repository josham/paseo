...
export function usePromptHistorySearch(input: {
  ...
  query: string;
  /**
   * The composer's text right now, for the value Escape restores. Defaults to
   * `query`, which can lag it by a frame where typing is published after paint.
   */
  getQuery?: () => string;
  replaceText: (text: string, selection?: { start: number; end: number }) => void;
  focusInput: () => void;
}): PromptHistorySearchController {
  const { enabled, entries, status, query, getQuery, replaceText, focusInput } = input;
  ...
  const open = useCallback(() => {
    ...
    restoreTextRef.current = getQuery?.() ?? query;
    setSelectedIndex(0);
    setIsOpen(true);
    focusInput();
    return true;
  }, [enabled, focusInput, getQuery, query]);
